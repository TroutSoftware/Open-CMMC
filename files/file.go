...
type FileInfo struct {
	*Listing
	...
	// Mark is the CUI marking attached to this file, if any. Populated
	// by the HTTP resource handler from the cmmc/marking store; this
	// struct does not own the mark, it carries it to the JSON response
	// so the SPA can render the badge without a second round-trip. The
	// cmmc/marking.Mark type is a string underneath, so keeping this
	// as plain string keeps the files package dependency-free.
