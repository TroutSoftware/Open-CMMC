...
export async function fetchURL(
  ...
  if (res.status < 200 || res.status > 299) {
    ...
    if (auth && res.status == 401) {
      // Session expired (idle lock, revoked, or stale signing key).
      // Kick the user through the re-auth flow AND silence the
      // error so callers don't show a scary "401 Unauthorized"
      // toast before the redirect lands. The thrown StatusError
      // carries a sentinel message so CustomToast can spot it too.
      logout("session-expired");
      const silent = new StatusError(
        "session-expired",
        res.status,
        true
      );
