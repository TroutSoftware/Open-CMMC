<template>
  <div>
    <!--
      CMMC: under OIDC, identity + credentials live in Keycloak, not
      here. The filebrowser "user" row exists only to carry per-user
      authorization state (scope, perms) keyed on the iss+sub pair.
      Hide fields that would mislead an operator into thinking they
      can set a password or rename a user from this screen.
    -->
    <p v-if="isOIDC" class="cmmc-identity-notice">
      <strong>Identity managed in Keycloak.</strong>
      Username, password, and MFA are owned by the identity provider.
      This form controls file-scope and authorization only; admin
      status is derived from the Keycloak group <code>cmmc-admins</code>
      on every login.
    </p>

    <p v-if="!isDefault && props.user !== null && !isOIDC">
    ...
    <p v-if="!isDefault && isOIDC && props.user !== null">
    ...
    <p v-if="!isDefault && !isOIDC && user.perm">
      <input
        type="checkbox"
