    ...
    <header-bar showMenu showLogo>
      ...
      <template #actions>
        <template v-if="!isMobile">
          ...
          />
          <action
            v-if="headerButtons.classifyCui"
            id="classify-cui-button"
            icon="label"
            label="Classify CUI"
            show="classify-cui"
          />
...
const headerButtons = computed(() => {
  // CMMC 3.8.4 — classify toolbar button visible to admins only.
  // Target is either: (a) the current folder when browsing without
  // a selection, or (b) the single selected item — file OR folder.
  // Per-file marks override the inherited folder mark for that one
  // file; backend's GetEffective walks ancestors so unmarked files
  // still inherit correctly. The PUT is still gated admin + fresh
  // MFA at the route layer.
  //
  // Permissions modal stays folder-only — ACLs apply to subtrees.
  ...
  if (authStore.user?.perm.admin) {
    if (req && req.isDir && (!sel || sel.length === 0)) {
      classifyVisible = true;
      permissionsVisible = true;
    } else if (req && req.items && sel && sel.length === 1) {
      classifyVisible = true;
      permissionsVisible = !!req.items[sel[0]]?.isDir;
    }
  }
  return {
