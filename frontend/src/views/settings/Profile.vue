<template>
  <div class="row">
    ...
    <div v-if="!noAuth" class="column">
      <!--
        Under OIDC, passwords live in Keycloak — changing a password
        here has no effect on next login. The form was a dead end that
        confused users. Replace with a note pointing to the real place
        to change credentials + manage MFA.
      -->
      <div v-if="isOIDC" class="card">
        <div class="card-title">
          <h2>{{ t("settings.changePassword") }}</h2>
        </div>
        <div class="card-content">
          <p class="cmmc-identity-notice">
            <strong>Managed in Keycloak.</strong>
            Your password and MFA device are set in your company's
            identity provider, not here. To change them, open your
            Keycloak account console or contact your ISSO. This
            cabinet verifies the credentials Keycloak produces; it
            never stores or resets them.
          </p>
        </div>
      </div>
