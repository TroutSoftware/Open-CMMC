...
func previewHandler(imgSvc ImgService, fileCache FileCache, enableThumbnails, resizePreview bool) handleFunc {
	return withUser(func(w http.ResponseWriter, r *http.Request, d *data) (int, error) {
		...
		// CMMC 3.8.4: preview of CUI = visual disclosure of CUI. Gate
		// the same as a raw download. Also note: previewCacheKey does
		// NOT include mark state, so previously-cached clean-file
		// thumbnails survive a mark change; the gate here blocks
		// serving those bytes once a mark is applied.
		if s := enforceCUIRead(r, d, "/"+vars["path"], "preview of CUI requires fresh MFA", false); s != 0 {
			return s, nil
		}
