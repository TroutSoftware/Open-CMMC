...
var publicShareHandler = withHashFile(func(w http.ResponseWriter, r *http.Request, d *data) (int, error) {
	...
	// CMMC 3.8.4 / 3.1.22: a share that was created on a clean file
	// must stop serving the moment the file is marked CUI. Public
	// callers have no step-up, so this is a hard 403. Applies to the
	// metadata endpoint too: even the filename/size of a CUI file is
	// a disclosure over an unauthenticated channel.
	if s := enforceCUIRead(r, d, file.Path, "public share metadata blocked", true); s != 0 {
		return s, nil
	}
...
var publicDlHandler = withHashFile(func(w http.ResponseWriter, r *http.Request, d *data) (int, error) {
	...
	if s := enforceCUIRead(r, d, file.Path, "public download blocked", true); s != 0 {
		return s, nil
	}
