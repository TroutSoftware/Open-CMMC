...
func setContentDisposition(w http.ResponseWriter, r *http.Request, file *files.FileInfo) {
	// CMMC 3.13.11 / XSS + same-origin CSRF defense — even when the
	// caller asked for inline rendering, content types that the
	// browser will execute as code (HTML, SVG, XHTML, XML) are
	// forced to `attachment`. A user-uploaded HTML page rendered
	// inline at the filebrowser origin can read the session cookie
	// (not after HttpOnly, but can still issue same-origin fetches
	// with credentials:include against any non-GET route) and
	// drive an authenticated CSRF. Forcing download defeats the
	// rendering path entirely.
	inline := r.URL.Query().Get("inline") == "true" && !isBrowserExecutable(file)
	if inline {
...
// isBrowserExecutable returns true for content that a browser would
// render as an active document — scripts inside get to issue same-
// origin fetches. Matched on MIME type first, filename extension
// second (MIME may be empty for files whose content-detection was
// off). The list intentionally errs long: safer to force download
// for an obscure type than to risk an inline render that shouldn't
// have been.
func isBrowserExecutable(file *files.FileInfo) bool {
	if file == nil {
		return false
	}
	mt := strings.ToLower(file.Type)
	switch {
	case strings.HasPrefix(mt, "text/html"),
		strings.HasPrefix(mt, "application/xhtml"),
		strings.HasPrefix(mt, "image/svg"),
		strings.HasPrefix(mt, "application/xml"),
		strings.HasPrefix(mt, "text/xml"),
		strings.HasPrefix(mt, "application/xslt"):
		return true
	}
	switch strings.ToLower(filepath.Ext(file.Name)) {
	case ".html", ".htm", ".xhtml", ".svg", ".xml", ".xslt":
		return true
	}
	// Belt-and-braces: a dot-less filename (Dockerfile, README,
	// attacker-uploaded "index") can't be ruled safe by extension
	// alone. If type detection didn't classify it (mt is empty),
	// force attachment so the browser can't MIME-sniff its way
	// into executing HTML-shaped text.
	if mt == "" && !strings.Contains(file.Name, ".") {
		return true
	}
	return false
}
...
var rawHandler = withUser(func(w http.ResponseWriter, r *http.Request, d *data) (int, error) {
	...
	// CMMC 3.5.3 / 3.8.4: CUI downloads require fresh MFA. See
	// enforceCUIRead — the single choke point for every read path.
	if s := enforceCUIRead(r, d, r.URL.Path, "raw download of CUI requires fresh MFA", false); s != 0 {
		return s, nil
	}
...
func rawDirHandler(w http.ResponseWriter, r *http.Request, d *data, file *files.FileInfo) (int, error) {
	...
	// CMMC 3.8.4: a zip of a tree inherits the strictest mark of any
	// file inside it. Scan children once; if any is CUI, apply the
	// read gate before we stream any archive bytes.
	if s := enforceCUIReadTree(r, d, filenames); s != 0 {
		return s, nil
	}
