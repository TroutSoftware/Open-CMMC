...
func tusPatchHandler(cache UploadCache) handleFunc {
	return withUser(func(w http.ResponseWriter, r *http.Request, d *data) (int, error) {
		...
		// Explicit Close — CMMC 3.14.2: the scan runs inside
		// EncryptingFS.writeFile.Close, so we can't defer-and-
		// swallow. Close error maps via errToStatus (RejectedError
		// → 422, ErrUnavailable → 503). See resource.go writeFile
		// for the matching pattern.

		if _, err := openFile.Seek(uploadOffset, 0); err != nil {
			_ = openFile.Close()
		...
		if closeErr := openFile.Close(); closeErr != nil {
			...
			if errors.As(closeErr, &rejected) {
				emitScanReject(r, d, r.URL.Path, rejected.Signature)
			} else if errors.Is(closeErr, scan.ErrUnavailable) {
				emitScanError(r, d, r.URL.Path, closeErr)
			}
