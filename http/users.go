...
func getUser(_ http.ResponseWriter, r *http.Request) (*modifyUserRequest, error) {
	...
	// CWE-400: cap the user-record JSON at 256 KiB. Rules + commands
	// + scope account for most of the size; 256 KiB is generous.
...
var userPostHandler = withAdmin(func(w http.ResponseWriter, r *http.Request, d *data) (int, error) {
	...
	log.Printf("user: %s, home dir: [%s].", logSafe(req.Data.Username), userHome)
...
// sensitiveUserFields lists req.Which values whose update requires a
// fresh MFA assertion on OIDC deployments. Preference fields
// (viewMode, locale, singleClick, redirectAfterCopyMove, dateFormat,
// hideDotfiles, aceEditorTheme) are intentionally absent — a "switch
// view" click must not force a re-auth.
var sensitiveUserFields = map[string]struct{}{
	"all":          {},
	"username":     {},
	"password":     {},
	"scope":        {},
...
var userPutHandler = withSelfOrAdmin(func(w http.ResponseWriter, r *http.Request, d *data) (int, error) {
	...
	// Fresh-MFA gate: only when a sensitive field is in req.Which, or
	// when req.Which is empty (the admin bulk-replace path, which
	// overwrites the full user record). No-op on non-OIDC deployments.
	requiresFresh := len(req.Which) == 0
	for _, f := range req.Which {
		if _, ok := sensitiveUserFields[strings.ToLower(f)]; ok {
			requiresFresh = true
			break
		}
	}
	if requiresFresh && !hasFreshMFA(r, d) {
		emitPrivRejectEvent(r, userIDString(d.user.ID), d.user.Username, "user-update touches sensitive field; requires fresh MFA")
